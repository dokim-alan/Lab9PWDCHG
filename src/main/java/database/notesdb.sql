DROP SCHEMA IF EXISTS `notes9db`;
CREATE SCHEMA IF NOT EXISTS `notes9db` DEFAULT CHARACTER SET latin1;
USE `notes9db`;

CREATE TABLE IF NOT EXISTS `notes9db`.`role` (
  `role_id` INT(11) NOT NULL,
  `role_name` VARCHAR(25) NOT NULL,
  PRIMARY KEY (`role_id`));

CREATE TABLE IF NOT EXISTS `notes9db`.`user` (
  ...
  CONSTRAINT `fk_user_role`
    FOREIGN KEY (`role`)
    REFERENCES `notes9db`.`role` (`role_id`)
);
...
INSERT INTO `user` (`email`,`active`,`first_name`,`last_name`,`password`,`role`)
	VALUES ('cprg352+alan@gmail.com', true, 'Barb','Barber', 'password', 2);
